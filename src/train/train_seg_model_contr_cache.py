...
from core.cache import Cache, BatchedPercentilePolicy, MaskTextCache, SimpleExpSmoothing
...
async def main() -> None:
    ...
    # update_policy = BatchedPercentilePolicy(metric=BinaryJaccardIndex().to(CONFIG["device"]), percentile=0.6)
    ...
    if True:
        ...
        mask_text_cache.cache = Cache.load(
            directory_path=Path("/home/olivieri/exp/data/torch_weights/cache/phase_3_cache_prefill_"),
            storage_device="cpu",
            memory_device="cuda"
        )
    else:
