...
from core.prompter import FastPromptBuilder, make_synthetic_diff_text
...
config = setup_config(BASE_CONFIG, Path('/home/olivieri/exp/src/train/seg/4-contrastive-B_PR_vs_dT/4.1-ft_synth_bootstrap/base/phase_2-config.yml'))
...
async def train_loop(
        ...
        neg_text_gen: NegativeTextGenerator,
    ...
    for epoch in range(start_epoch, seg_train_config["num_epochs"]):
            ...
            if seg_train_with_text_config['with_text']:
                ...
                for i, uid in enumerate(uids):
                        ...
                        for pos_c in sign_classes:
                            cs_text = make_synthetic_diff_text("/home/olivieri/exp/data/private/prompts_data/synth_text/v1.json", gts_down[i].squeeze(0) == pos_c, prs_down[i].squeeze(0) == pos_c, class_names[pos_c])
                            cs_answer_dict[uid][pos_c] = cs_text
                
                # --- VLE --- #

                with autocast():
                    
                    uids_to_update = list(cs_dict.keys())
                    sign_classes_to_update = list(cs_dict.values())
                    ...
                    b_global_image_token = torch.cat([b.unsqueeze(0).expand(len(cs_dict[uid]), -1) for b, uid in zip(b_global_image_token, cs_dict.keys()) if uid in uids_to_update]) # (P, D)
                    ...
                    group_indices_for_pos_pairs = [[i]*len(pos_classes) for i, pos_classes in enumerate(cs_dict.values())]
                ...
                cs_mult = cs_counter/(train_dl.batch_size)
            else:
        ...
        if val_metrics_score['mIoU'] > best_val_mIoU or epoch == 4:
            best_val_mIoU = val_metrics_score['mIoU']
        ...
        if epoch == 4:
            log_manager.log_line(f"Epoch {epoch+1} reached.")
            break
