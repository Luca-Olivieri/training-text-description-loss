...
from models.vle import VLE_REGISTRY, VLEncoder, MapComputeMode
from train.loss import xen_rescaler, lin_rescale_fn, log_rescale_fn, exp_rescale_fn, pow_rescale_fn
from core.prompter import FastPromptBuilder
from core.logger import LogManager
from core.path import get_mask_prs_path
from core.viz import get_layer_numel_str, create_diff_mask, overlay_map
from core.utils import clear_memory, compile_torch_model, subsample_sign_classes, blend_tensors, nanstd
        ...
        for step, (uids, scs_img, gts) in enumerate(train_dl):
            ...
            if SEG_WITH_TEXT_CONFIG['with_text']:
                ...
                with autocast():
                    ...
                    for i, (uid, sc, gt, pr) in enumerate(zip(uids, scs_down, gts_down, prs_down)):
                        ...
                        if uid in cs_answer_dict.keys():
                            ...
                            if isinstance(SEG_CONFIG['image_size'], int):
                                map = torch.ones(SEG_CONFIG['image_size'], SEG_CONFIG['image_size'])
                                map = map.to(CONFIG['device'])/torch.pow(torch.tensor(SEG_CONFIG['image_size']), 2)
                                # map = torch.zeros(SEG_CONFIG['image_size'], SEG_CONFIG['image_size']).to(CONFIG['device'])
                            if isinstance(SEG_CONFIG['image_size'], (tuple, list)):
                                map = torch.ones(SEG_CONFIG['image_size'])
                                map = map.to(CONFIG['device'])/(SEG_CONFIG['image_size'][0]*SEG_CONFIG['image_size'][1])
                                # map = torch.zeros(SEG_CONFIG['image_size'])
                            cs_maps.append(map)
                    ...
                    _rescale_mult = batch_loss.mean()/old_batch_loss.mean().detach()
...
async def main() -> None:
    ...
    mask_text_cache.cache = Cache.load(
        directory_path=Path("/home/olivieri/exp/data/torch_weights/cache/from_start_prefill_cache"),
        storage_device="cpu",
        memory_device="cuda"
    )
