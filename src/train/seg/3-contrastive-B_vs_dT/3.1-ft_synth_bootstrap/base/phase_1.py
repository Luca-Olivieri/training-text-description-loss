...
config = setup_config(BASE_CONFIG, Path('/home/olivieri/exp/src/train/seg/3-contrastive-B_vs_dT/3.1-ft_synth_bootstrap/base/phase_1-config.yml'))
...
async def train_loop(
    ...
    for epoch in range(start_epoch, seg_train_config["num_epochs"]):
                ...
                with autocast():
                    ...
                    for uid, cs_a in cs_answer_dict.items():
                        if uid in list(cs_dict_to_update.keys()):
                            ...
                            cs_global_text_tokens[uid] = torch.empty(0, 960, device=config['device']) # no text for this image
                    ...
                    batch_loss: torch.Tensor = seg_batch_loss*(1. - seg_train_with_text_config['loss_lam']) + aux_batch_loss*seg_train_with_text_config['loss_lam'] # lambda coefficient
        ...
        if val_metrics_score['mIoU'] > best_val_mIoU or epoch == 19:
            best_val_mIoU = val_metrics_score['mIoU']
