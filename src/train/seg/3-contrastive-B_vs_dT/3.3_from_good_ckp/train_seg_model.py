        ...
        for step, (uids, scs_img, gts) in enumerate(train_dl):
            ...
            if seg_train_with_text_config['with_text']:
                ...
                with autocast():
                    ...
                    with torch.no_grad():
                        flat_cs_vle_txt_output = vle.encode_and_project_texts(flat_cs_texts) # (P, 512)
                    
                    cs_global_text_token = flat_cs_vle_txt_output.global_text_token # (P, 512)
...
async def main() -> None:
    ...
    if seg_train_with_text_config['with_cache']:
        cache = Cache(storage_device='cpu', memory_device=config['device'])
    ...
    vle.logit_scale = nn.Parameter(torch.tensor(0.0, device=config['device']))
    vle.logit_bias = nn.Parameter(torch.tensor(0.0, device=config['device']))

    del vle.model.visual_proj # not needed
    clear_memory()
