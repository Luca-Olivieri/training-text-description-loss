...
if __name__ == '__main__':
    ...
    def test_GroupedSlackedNegativeSigLipLoss() -> None:
        """
        Creates dummy data and runs one forward pass of the loss function
        to demonstrate its logic. This function implements Example 1.
        """
        print("--- Running GroupedSlackedNegativeSigLipLoss Example ---")

        # --- 1. Define Parameters ---
        N = 4  # Total images in batch
        P = 3  # Total positive texts in batch
        D = 128 # Embedding dimension
        M = 5  # Target number of negatives
        
        # --- 2. Instantiate Loss Function ---
        loss_fn = GroupedSlackedNegativeSigLipLoss(ignore_no_positive_samples=True)
        loss_fn_no_pos = GroupedSlackedNegativeSigLipLoss(ignore_no_positive_samples=False)
        ...
        # Features
        image_features = torch.randn(N, D)
        positive_text_features = torch.randn(P, D)
        slacked_negative_text_features = torch.randn(N, M, D)

        # Mappings for pairs and groups (implements the logic of Example 1)
        # Pair mappings: t0->i0, t1->i1, t2->i2
        image_indices_for_pos_texts = torch.tensor([0, 1, 2], dtype=torch.long)
        # Group mappings: (i0,t0)->G0, (i1,t1)->G0, (i2,t2)->G1
        group_indices_for_pos_pairs = torch.tensor([0, 0, 1], dtype=torch.long)
        
        # Learnable parameters
        logit_scale = torch.nn.Parameter(torch.tensor(10.0))
        logit_bias = torch.nn.Parameter(torch.tensor(-5.0))
        
        print("\n--- Setup ---")
        print(f"Total Images (N): {N}")
        print(f"Positive Pairs (P): {P}")
        print(f"Target Negatives (M): {M}")
        print(f"In-batch negatives available for images with a positive: P-1 = {P-1}")
        print(f"Pair Mappings: {image_indices_for_pos_texts.tolist()}")
        print(f"Group Mappings: {group_indices_for_pos_pairs.tolist()}")
        print("-" * 20)
        ...
        loss_dict = loss_fn(
            ...
            image_indices_for_pos_texts,
            group_indices_for_pos_pairs,
            slacked_negative_text_features,
            logit_scale,
            logit_bias,
            output_dict=True
        )
        loss_no_pos_dict = loss_fn_no_pos(
            image_features,
            positive_text_features,
            image_indices_for_pos_texts,
            group_indices_for_pos_pairs,
        ...
        final_loss = loss_dict['contrastive_loss']
        final_no_pos_loss = loss_no_pos_dict['contrastive_loss']

        print(f"\n--- Result ---")
        print(f"Computed Loss: {final_loss.item():.4f}")
        print(f"Computed Loss (no pos): {final_no_pos_loss.item():.4f}")
        print("------------------------------------------------------\n")
    ...
    test_GroupedSlackedNegativeSigLipLoss()
