...
from viz import get_layer_numel_str, create_cs_masks
...
async def train_loop(
    ...
    for epoch in range(start_epoch, SEG_TRAIN_CONFIG["num_epochs"]):
        ...
        accum_img_features, accum_cs_global_text_tokens, accum_cs_masks_visual_tokens = [], {}, {}
        seg_batch_loss = None
        cs_counter = 0
        ...
        for step, (uids, scs_img, gts) in enumerate(train_dl):
            ...
            if SEG_WITH_TEXT_CONFIG['with_text']:
                ...
                scs_by_uid = {uid: pr for uid, pr in zip(uids, scs_down)}
                ...
                with autocast():
                    ...
                    with torch.no_grad():
                        ...
                        for uid, cs_a in cs_answer_dict.items():

                            cs_masks = create_cs_masks(scs_by_uid[uid], mask=prs_by_uid[uid].squeeze(0), sign_classes=list(cs_a.keys()), alpha=0.55)

                            cs_masks = list(cs_masks.values())
                            cs_masks = vle.preprocess_images(cs_masks)

                            cs_texts = list(cs_a.values()) # gather the text for each pos. class of this image
                            cs_texts = vle.preprocess_texts(cs_texts)
                            cs_vle_output = vle.encode_and_project(images=cs_masks, texts=cs_texts, broadcast=False, pool=False)

                            cs_global_text_token = cs_vle_output.global_text_token.squeeze(1) # [N_cs, D]
                            ...
                            accum_cs_global_text_tokens |= {uid: cs_global_text_token.detach()}
                            accum_cs_masks_visual_tokens |= {uid: cs_vle_output.global_image_token.detach()}
                    ...
                    with autocast():
                        ...
                        for i, uid in enumerate(uids):
                            ...
                            if uid in accum_cs_global_text_tokens.keys():
                                ...
                                cs_masks_visual_tokens = accum_cs_masks_visual_tokens[uid]

                                cs_global_token = cs_global_text_token + cs_masks_visual_tokens
                                cs_global_token = segmodel.model.bottleneck_adapter.mlp(cs_global_token)

                                aux_batch_loss += aux_criterion(
                                    image_features=bottleneck_out,
                                    text_features=cs_global_token,
