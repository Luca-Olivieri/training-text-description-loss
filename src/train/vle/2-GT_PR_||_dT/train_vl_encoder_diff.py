...
from core.data import JSONLDataset, ImageDataset, ImageCaptionDataset, COCO2017SegDataset, VOC2012SegDataset, crop_image_preprocess_image_text_batch, crop_augment_preprocess_batch, apply_classmap
from models.vle import VLE_REGISTRY, VLEncoder
from core.viz import get_layer_numel_str, blend_tensors
from core.utils import compile_torch_model, get_torch_gen, clear_memory
...
def train_loop(
    ...
    for epoch in range(start_epoch, VLE_TRAIN_CONFIG["num_epochs"]):
        ...
        for step, (images, texts, texts_metadata) in enumerate(train_dl):
            ...
            ovr_concat_masks = format_diff_images(scs_img, gts, prs, pos_classes)
            ...
            with autocast():
                ...
                # rhs = torch.cat([vle_output.global_text_token.squeeze(1), pr_global_image_token], dim=-1)
...
def evaluate_diff(
        ...
        with torch.no_grad():
            ...
            for step, (images, texts, texts_metadata) in enumerate(dl):
                ...
                scs_img: torch.Tensor = (scs_img*255).to(torch.uint8)

                gts: torch.Tensor = gts.unsqueeze(1)
...
def format_diff_images(
    ...
    pos_classes = torch.tensor(pos_classes).to(prs.device).view(-1, 1, 1, 1).repeat(2, 1, 1, 1)

    concat_masks = torch.cat([gts, prs], dim=0)

    pos_class_concat_masks = (concat_masks == pos_classes).to(torch.uint8)*255

    ovr_concat_masks = (blend_tensors(scs_img.repeat(2, 1, 1, 1), pos_class_concat_masks.expand(-1, 3, -1, -1), 0.55)/255.).float()

    return ovr_concat_masks
...
def main() -> None:
    ...
    # checkpoint_dict = torch.load('/home/olivieri/exp/data/torch_weights/vle/flair/full_training/flair-flair-cc3m-recap.pt-text_proj_L_b256_250902_2047.pth', map_location=CONFIG['device'])
    # vle.model.load_state_dict(checkpoint_dict['model_state_dict'], strict=False)
    # checkpoint_dict = None

    # vle.set_vision_trainable_params(['proj', 'visual_proj', 'vision_adapter'])
    vle.set_vision_trainable_params(['text_proj', 'concat_adapter'])
