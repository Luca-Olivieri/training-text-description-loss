...
from core.utils import clear_memory, compile_torch_model, subsample_sign_classes, nanstd
from core.data import VOC2012SegDataset, crop_augment_preprocess_batch, apply_classmap
...
from torchmetrics.classification import MulticlassAccuracy, MulticlassJaccardIndex, BinaryJaccardIndex, BinaryAccuracy
...
SEG_CONTR_CONFIG['loss_lam'] = 0.0

SEG_TRAIN_CONFIG['lr_schedule']['base_lr'] = 0.0
    ...
    for epoch in range(start_epoch, SEG_TRAIN_CONFIG["num_epochs"]):
        ...
        for step, (uids, scs_img, gts) in enumerate(train_dl):
            ...
            if SEG_WITH_TEXT_CONFIG['with_text']:
                ...
                cs_answer_list = [{"img_idx": img_idx, "content": {pos_c: "..." for pos_c in cs_prompts[i].keys()}} for i, img_idx in enumerate(batch_idxs)]

                cs_splitted_bool_masks = {uid: fast_prompt_builder.get_cs_splitted_masks(prs_down[i]) for i, uid in enumerate(uids)}
                
                batch = {f"{uid}-{pos_c}": (bool_mask, "...") for uid, cs_bool_mask in cs_splitted_bool_masks.items() for pos_c, bool_mask in cs_bool_mask.items()}
            ...
            if is_accum_step or is_last_batch:
                ...
                if global_step % SEG_TRAIN_CONFIG['log_every'] == 0:
                    ...
                    metric_diffs_values = torch.Tensor(list(mask_text_cache.get_metric_diffs(batch).values()))
                    train_metrics_score = {'aux_loss': aux_batch_loss} | train_metrics_score | {'metric_diff_mean': metric_diffs_values.nanmean(), 'metric_diff_std': nanstd(metric_diffs_values, dim=0)}
        ...
        mask_text_cache.cache.save(directory_path=Path(f"/home/olivieri/exp/data/torch_weights/cache/{SEG_TRAIN_CONFIG['exp_name']}"))
