...
from core._types import Optional, ImageMediaMetadata
from pathlib import Path
import re
import time

from core.prompter import PromptBuilder

config = setup_config(BASE_CONFIG, Path('/home/olivieri/exp/src/annot_forms/annot_form_config.yml'))
...
def write_prompt_to_gdoc(
        gdoc_id: str,
        prompt: list,
        img_ids_sizes: list[tuple[int, int]]
) -> None:
    """
...
def write_annot_doc(
    ...
    for i, query_idx in enumerate(query_idxs):
        time.sleep(5)
        ex_1 = get_answer_objects(..., query_idx)[query_idx]
        ex_2 = get_answer_objects(..., query_idx)[query_idx]
        ex_3 = get_answer_objects(..., query_idx)[query_idx]
